...
def generate_pass(number):
    # 获取对应数字的枚举值
    all_actions = list(Actions_LLVM_10_0_0)
    max_index = len(all_actions) - 1
    
    # 确保number在有效范围内
    if not isinstance(number, int) or number < 0:
        print(f"警告: 数字 {number} 无效，使用默认值0")
        number = 0
    elif number > max_index:
        print(f"警告: 数字 {number} 超出范围 (0-{max_index})，使用模运算")
        number = number % (max_index + 1)
    
    try:
        # 使用枚举名获取对应的值
        action = all_actions[number]
        return [action.value]
